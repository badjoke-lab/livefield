...
export function renderBattleLinesPage(root: HTMLElement): void {
  ...
  root.innerHTML = `
    ${renderHeader("battle-lines", kickSiteConfig)}
    ${renderHero({
      eyebrow: "RIVALRIES",
      title: "Rivalry Radar",
    ...
    try {
      ...
      if (!silent) {
        ...
        const escaped = detail
          ...
          .replaceAll("<", "&lt;")
          .replaceAll(">", "&gt;")
        console.error("[kick battle-lines] runLoad failed", error)
        content.innerHTML = `<section class="card battle-state-card battle-state-card--error"><h2>Battle lines unavailable</h2><p>Could not refresh battle lines right now. Try again shortly.</p><pre class="battle-error-detail">${escaped}</pre></section>`
      } else {
...
export { renderBattleLinesPage as renderKickBattleLinesPage }
