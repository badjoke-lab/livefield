...
function formatActivityState(node: HeatmapNode): string {
  if (!node.activityAvailable) return `Unavailable${node.activityUnavailableReason ? ` (${node.activityUnavailableReason})` : ""}`
  if (node.commentsPerMin === 0) return "Sampled (0 activity)"
  return "Sampled (activity detected)"
}
