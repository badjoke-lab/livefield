...
function getMomentumClass(momentum: number): string {
  if (momentum >= 0.15) return "tile-node--momentum-3"
  if (momentum >= 0.08) return "tile-node--momentum-2"
  if (momentum >= 0.03) return "tile-node--momentum-1"
  return "tile-node--momentum-0"
}

function getActivityBadge(node: HeatmapNode): string {
  const state = getActivityState(node)
  if (state === "active") return '<span class="tile-node__badge tile-node__badge--active">active</span>'
  if (state === "sampled_zero") return '<span class="tile-node__badge tile-node__badge--sampled-zero">sampled 0</span>'
...
export function mountTileMockRenderer(
  ...
  root.innerHTML = cells
    .map(({ node, rect }) => {
      ...
      const showMeta = rect.w > 20 && rect.h > 18
      const showTitle = rect.w > 13 && rect.h > 12
      return `<button type="button" class="tile-node ${getMomentumClass(node.momentum)} tile-node--${state} ${selected ? "tile-node--selected" : ""}" style="left:${rect.x.toFixed(3)}%;top:${rect.y.toFixed(3)}%;width:${rect.w.toFixed(3)}%;height:${rect.h.toFixed(3)}%;" data-streamer-id="${escapeHtml(node.streamerId)}"><span class="tile-node__tone" aria-hidden="true"></span>${showTitle ? `<span class="tile-node__name">${escapeHtml(node.name)}</span>` : ""}${showMeta ? `<span class="tile-node__viewers">${node.viewers.toLocaleString()}</span>${getActivityBadge(node)}` : ""}</button>`
    })
    .join("")
